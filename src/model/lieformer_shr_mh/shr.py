...
Wigner-6j recoupling, source-local tensor products, grouped edge aggregation,
...
from ._shared_paths import OuterPath, SHRBranch, build_outer_paths, recouple_paths
from .aggregation import grouped_aggregate
from ._shared_primitives import center_positions, component_tensor_product, regular_solid_harmonics, scatter_sum
...
class SHRPlan:
    ...
    def manifest(self) -> dict[str, object]:
        return {
            ...
            "branches": [
                {
                    "path_id": b.outer_path_id,
                    "l_in": b.input_degree,
                    "ell": b.geometry_degree,
                    "l_out": b.output_degree,
                    "a": b.source_degree,
                    "u": b.target_degree,
                    "f": b.intermediate_degree,
...
class SparseHarmonicRecoupling(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if aggregation_backend not in ("scatter", "sparse", "block_sparse"):
            raise ValueError("aggregation_backend must be scatter, sparse or block_sparse")
        self.aggregation_backend = aggregation_backend
        if feature_lmax is not None:
            if l_in_max is None:
                l_in_max = int(feature_lmax)
            if l_out_max is None:
                l_out_max = int(feature_lmax)
        if input_degrees is None:
            if l_in_max is None:
                raise ValueError("provide feature_lmax/l_in_max or input_degrees")
            input_degrees = range(int(l_in_max) + 1)
        if output_degrees is None:
            if l_out_max is None:
                l_out_max = int(l_in_max if l_in_max is not None else max(input_degrees))
            output_degrees = range(int(l_out_max) + 1)
        if isinstance(input_degrees, int):
            input_degrees = range(input_degrees + 1)
        if isinstance(output_degrees, int):
            output_degrees = range(output_degrees + 1)
        ...
        if outer_path_subset is None:
            outer_path_subset = outer_paths
        ...
        self.path_weights = torch.nn.ParameterDict(
            {str(p.path_id): torch.nn.Parameter(torch.empty(self.in_channels, self.out_channels)) for p in paths}
        )
    ...
    def symbolic_plan(self) -> SHRPlan:
        return self.plan

    def moment_table(self) -> tuple[SHRBranch, ...]:
        return self.plan.branches

    def _validate(
        self, features: Mapping[int, Tensor], positions: Tensor, edge_index: Tensor, alpha: Tensor, batch: Tensor | None
    ) -> Tensor:
        ...
        for l in self.input_degrees:
            if l not in features:
                raise KeyError(f"features are missing degree {l}")
            expected = (n, 2 * l + 1, self.in_channels)
            if tuple(features[l].shape) != expected:
                raise ValueError(f"degree-{l} features must have shape {expected}")
            if features[l].dtype != positions.dtype or features[l].device != positions.device:
                raise ValueError("features and positions must share dtype/device")
        if edge_index.ndim != 2 or tuple(edge_index.shape[:1]) != (2,) or edge_index.dtype != torch.long:
            raise ValueError("edge_index must have shape (2,E) and dtype torch.long")
        if batch is not None:
            if batch.ndim != 1 or batch.shape[0] != n or batch.dtype != torch.long or batch.device != positions.device:
                raise ValueError("batch must be a long tensor of shape (N,)")
            if edge_index.shape[1] and not torch.equal(
                batch.index_select(0, edge_index[0]), batch.index_select(0, edge_index[1])
            ):
                raise ValueError("edges must not connect graphs")
        if alpha.ndim == 1:
            alpha = alpha[:, None]
        if (
            alpha.ndim != 2
            or alpha.shape[0] != edge_index.shape[1]
            or alpha.shape[1] < 1
            or self.in_channels % alpha.shape[1]
            or alpha.dtype != positions.dtype
            or alpha.device != positions.device
        ):
            raise ValueError("alpha must be (E,H), H must divide input channels, with positions dtype/device")
        return alpha
    ...
    def build_local_moments(
        ...
        *,
        ...
        for l, a, f in keys:
            ...
            source_parts.append(src.reshape(pos.shape[0], -1))
            widths.append(src.shape[1] * src.shape[2])
        src_i, dst_i = edge_index
        stacked = torch.cat(source_parts, dim=-1)
        aggregated = grouped_aggregate(stacked, alpha, edge_index, nt, self.in_channels, self.aggregation_backend)
        out, offset = {}, 0
    ...
    def decode_moments(
        self,
        moments: Mapping[tuple[int, int, int], Tensor],
        positions: Tensor,
        *,
        batch: Tensor | None = None,
        centered_positions: Tensor | None = None,
        ...
        grouped: dict[tuple[int, int, int], dict[tuple[int, int, int], list[tuple[int, float]]]] = defaultdict(
            lambda: defaultdict(list)
        )
        for b in self.plan.branches:
            grouped[(b.output_degree, b.intermediate_degree, b.target_degree)][
                (b.input_degree, b.source_degree, b.intermediate_degree)
            ].append((b.outer_path_id, b.coefficient))
        n = pos.shape[0]
        for (q, f, u), terms in grouped.items():
            ...
            for key in keys:
                w = None
                for pid, coeff in terms[key]:
                    w = coeff * self.weight_for(pid) if w is None else w + coeff * self.weight_for(pid)
                weights.append(w)
            projected = torch.bmm(
                dest.reshape(len(keys), n * (2 * q + 1), self.in_channels), torch.stack(weights)
            ).reshape(len(keys), n, 2 * q + 1, self.out_channels)
    ...
    def forward(
        ...
        *,
        ...
        moments = self.build_local_moments(
            features,
            positions,
            edge_index,
            alpha,
            batch,
            centered_positions=pos,
    ...
    def forward_flat(
        self, a_hi: Tensor, positions: Tensor, edge_index: Tensor, alpha: Tensor, batch: Tensor | None = None
    ) -> Tensor:
    ...
    def edgewise_reference(
        ...
        # Weight input channel groups before the dense path matrix; these do not commute.
        for p in self.paths:
            coupled = component_tensor_product(
                features[p.input_degree].index_select(0, src),
                geom[p.geometry_degree],
                p.input_degree,
                p.geometry_degree,
                p.output_degree,
            )
            out[p.output_degree] = out[p.output_degree] + scatter_sum(
                (coupled * alpha.repeat_interleave(self.in_channels // alpha.shape[1], dim=1)[:, None, :])
                @ self.weight_for(p),
                dst,
                pos.shape[0],
            )
