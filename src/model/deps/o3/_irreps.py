...
change:
- Irreps.__getitem__ , make it accept int only and lways return _MulIr;
- _MulIr.__new__, correct type check;
- _MulIr.__getitem__, strict index value;
...
class _MulIr(tuple):
    def __new__(cls, mul: Union[int, "_MulIr"], ir: Optional[Irrep] = None):
        if ir is None:
            assert isinstance(mul, tuple) and len(mul) == 2, "mul must be a tuple of length 2"
            mul, ir = mul
    ...
    def __getitem__(self, index) -> Union[int, Irrep]:  # pylint: disable=useless-super-delegation
        if index in (0, 1):
            return super().__getitem__(index)
        else:
            raise ValueError("index must be in (0,1) but got: {index}")
...
class Irreps(tuple):
    ...
    def __new__(cls, irreps=None) -> "Irreps":
    ...
    def __iter__(self) -> Iterator[_MulIr]:  # 假设元素是 int 类型
        return super().__iter__()

    def __getitem__(self, i: int) -> _MulIr:
        if isinstance(i, int):
            x: _MulIr = super().__getitem__(i)
            return x
        else:
            raise TypeError(f"unsupported {type(i)}")
